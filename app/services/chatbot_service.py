...
def process_chat_request(data):
    ...
    # --- Rule-based ---
    if any(k in msg for k in ["address", "located", "location"]):
    ...
    if "vision" in msg:
        return build_response(f"Our vision is: {vision}")
    if "mission" in msg:
        return build_response(f"Our mission is: {mission}")

    if any(k in msg for k in ["office", "offices"]):
        return build_response(f"Our offices are located in: {offices}.")

    if any(k in msg for k in ["core values", "values", "principles"]):
        return build_response(f"Our core values are: {core_values}.")
